...
snarkjs fflonk setup build/${CIRCUIT}.r1cs ptau/powersOfTau28_hez_final_${PTAU}.ptau build/${CIRCUIT}_final.zkey
...
snarkjs fflonk prove build/${CIRCUIT}_final.zkey build/${CIRCUIT}_js/witness.wtns build/proof.json build/public.json

echo "----- Verify the proof -----"
# Verify the proof
snarkjs fflonk verify build/verification_key.json build/public.json build/proof.json

echo "----- Generate Solidity verifier -----"
# Generate a Solidity verifier that allows verifying proofs on Ethereum blockchain
snarkjs zkey export solidityverifier build/${CIRCUIT}_final.zkey build/${CIRCUIT}FflonkVerifier.sol
# Update the solidity version in the Solidity verifier
sed -i "s/>=0.7.0 <0.9.0;/^0.8.4;/g" build/${CIRCUIT}FflonkVerifier.sol
# Update the contract name in the Solidity verifier
sed -i "s/contract FflonkVerifier/contract ${CIRCUIT^}FflonkVerifier/g" build/${CIRCUIT}FflonkVerifier.sol
